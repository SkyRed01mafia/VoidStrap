...
print("[VoidStrap] Parte 1/6 carregada.")--====================================================================
-- COMPONENTE: SLIDER
--====================================================================
...
print("[VoidStrap] Parte 2A/6 carregada.")--====================================================================
-- COMPONENTE: SECTION
--====================================================================
...
local function toggleRow(parent, rotulo, inicial, aoMudar, ordem)
    ...
    local sub = criar("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "Ativa / desativa essa funcao",
...
--====================================================================
-- BOTOES DA TOPBAR (Fechar / Minimizar)
--====================================================================
...
local BotaoFechar    = botaoTopBar("X", -48)
...
print("[VoidStrap] Parte 2B/6 carregada.")--====================================================================
-- MÓDULO: GRAMA
--====================================================================
local GrassModule = {}
...
print("[VoidStrap] Parte 3/6 carregada.")--====================================================================
...
do
    ...
    local nomes = { "Padrao", "Noite", "Espaco", "Vermelho", "Roxo", "Personalizado" }
...
do
    ...
    local info = criar("TextLabel", {
        Size = UDim2.new(1, 0, 0, 40),
        BackgroundTransparency = 1,
        Text = "Altera apenas o FieldOfView da camera local. Reversivel e seguro.",
        Font = Enum.Font.Gotham,
        TextSize = 11,
        TextColor3 = TemaAtivo.Sub,
        TextWrapped = true,
...
do
    ...
    local info = criar("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "Funciona em campos de Terrain e em partes com material Grass. Local e reversivel.",
        Font = Enum.Font.Gotham,
        TextSize = 11,
        TextColor3 = TemaAtivo.Sub,
        TextWrapped = true,
...
do
    ...
    local info = criar("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "Controla o horario (ClockTime). Valores acima de 24 reiniciam o ciclo automaticamente.",
        Font = Enum.Font.Gotham,
        TextSize = 11,
        TextColor3 = TemaAtivo.Sub,
        TextWrapped = true,
    ...
    buttonRow(resetSec, "Restaurar iluminacao original", function()
        LightingModule.restaurar()
        notificar("Iluminacao restaurada", "good")
    end, 1)
end
...
do
    ...
    local info = criar("TextLabel", {
        Size = UDim2.new(1, 0, 0, 48),
        BackgroundTransparency = 1,
        Text = "Ultra: mantem tudo. Balanced: desativa sombras. Performance: desativa sombras e luz. Potato: melhor FPS.",
        Font = Enum.Font.Gotham,
        TextSize = 11,
        TextColor3 = TemaAtivo.Sub,
        TextWrapped = true,
...
do
    ...
    toggleRow(sec, "Animacoes", Estado.Settings.AnimationsEnabled, function(on)
        ...
        notificar(on and "Animacoes ativadas" or "Animacoes desativadas",
                  on and "good" or "bad")
    end, 1)
    ...
    buttonRow(resetSec, "Descarregar VoidStrap", function()
        if _G.VoidStrapUnload then _G.VoidStrapUnload() end
        ScreenGui:Destroy()
        notificar("VoidStrap descarregado", "bad")
    end, 1)
end

-- Seleciona a primeira aba por padrao
if next(Tabs) then
    selecionarAba(next(Tabs))
end

print("[VoidStrap] Parte 4/6 carregada.")--====================================================================
-- PARTE 5A — CHARS
--====================================================================
do
    ...
    local infoChars = criar("TextLabel", {
        Size = UDim2.new(1, 0, 0, 40),
        BackgroundTransparency = 1,
        Text = "Clique em um char para enviar o comando :char NOME no chat automaticamente.",
        Font = Enum.Font.Gotham,
        TextSize = 11,
        TextColor3 = TemaAtivo.Sub,
        TextWrapped = true,
...
do
    ...
    local secCor = section("Aparencia da Bola")
    secCor.Parent = pagCB
...
print("[VoidStrap] Parte 5/6 carregada.")--====================================================================
-- PARTE 6A — BOOM BOX
--====================================================================
do
    local EstadoBB = {
        ...
        if on then
            ...
            RC_Conn = RunService.Heartbeat:Connect(function()
                ...
                if (bola.Position - perna.Position).Magnitude <= EstadoRC.Distance then
                    ...
                    if EstadoRC.Mode == "Empurrao Continuo" then
                        pcall(function()
...
print("[VoidStrap] Parte 6A/6 carregada.")--====================================================================
-- PARTE 6B — TRAIL NO PERSONAGEM
--====================================================================
do
    local EstadoTR = {
    ...
    dropdownRow(secTR, "Textura", nomesT, "Padrao", function(opt)
        EstadoTR.Textura = opt
        if EstadoTR.Enabled then criarTrail() end
    end, 3)

    sliderRow(secTR, "Duracao (x10)", 1, 50, 10, function(v)
...
do
    ...
    sliderRow(secTO, "Forca da Curva (x10)", 1, 30, 10, function(v)
        EstadoTO.Forca = v / 10
    end, 3)

    local secBtnTO = section("Botao Flutuante (Mobile)")
    secBtnTO.Parent = pagTO

    toggleRow(secBtnTO, "Mostrar Botao", true, function(on)
    ...
    toggleRow(secBtnTO, "Travar Botao no Lugar", false, function(on)
        EstadoTO.BtnTravado = on
        notificar(on and "Botao travado" or "Botao liberado",
                  on and "good" or "bad")
    end, 2)

    local secResetTO = section("Restaurar")
    secResetTO.Parent = pagTO
    buttonRow(secResetTO, "Desativar Tote e remover botao", function()
...
--====================================================================
-- FINAL — Notificacao de carregamento
--====================================================================
task.wait(0.5)
notificar("VoidStrap " .. VERSAO .. " carregado!", "good")

print("[VoidStrap] Parte 6B/6 carregada. Script completo!")
